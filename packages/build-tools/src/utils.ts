    ...
    try {
        ...
        while (parentDir !== path.dirname(parentDir)) {
            ...
            try {
                ...
                const packageJsonContents = await readFile(potentialPackageJsonPath, 'utf-8');
...
export async function saveBuildData(buildDataPath: string, checksum: string, version: string) {
    try {
        const checksumData = {
            version,
            checksum
